...
def main(rank, args):
    ...
    if (
        ...
        and getattr(args, 'scene_gate_version', 'v5') in {'v5', 'v5_legacy'}
    ):
    ...
    param_dicts = []
    if clip_params:
        param_dicts.append({"params": clip_params, "lr": args.lr_vit})
    ...
    if other_params:
        param_dicts.append({"params": other_params, "lr": args.lr_head})
    ...
    if resume_training and checkpoint is not None:
        ...
        if getattr(args, 'override_resume_scene_gate_lr', False):
            ...
            for index in gate_group_indices:
                ...
                if index < len(lr_scheduler.base_lrs):
                    lr_scheduler.base_lrs[index] = new_gate_lr
                if (
                    hasattr(lr_scheduler, '_last_lr')
...
if __name__ == '__main__':
    ...
    if local_rank == 0:
        # Use args.output_dir as name or a unique run name
        ...
        wandb.init(project='LAIN', name=run_name)
