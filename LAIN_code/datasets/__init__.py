...
def resolve_hico_image_root(image_root, partition):
    base = image_root or ''
    if os.path.basename(os.path.normpath(base)) == partition:
        return base
    if os.path.basename(os.path.normpath(base)) == 'images':
        return os.path.join(base, partition)
    return os.path.join(base, 'images', partition)
...
class DataFactory(Dataset):
    def __init__(self, name, partition, data_root, clip_model_name, zero_shot=False, zs_type='rare_first',
        ...
        if name == 'hicodet':
            ...
            hico_image_root = resolve_hico_image_root(
                getattr(args, 'hico_image_root', os.path.join(data_root, 'hico_20160224_det')),
                partition
            )
            print(f'[INFO] HICO image root ({partition}): {hico_image_root}')
            self.dataset = HICODet(
                root=hico_image_root,
