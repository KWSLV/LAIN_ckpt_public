...
# Keep every effective parameter from the best run. Only SceneGate changes:
# centered_tanh removes the common negative bias before tanh, init_std starts
# the final gate layer near zero, and alpha controls the scene residual strength.
...
torchrun \
  ...
  --dataset hicodet \
  --zs --zs_type unseen_object --num_classes 117 \
  ...
  --scene_gate_alpha "${SCENE_GATE_ALPHA:-0.05}" \
  --scene_gate_activation centered_tanh --scene_gate_init_std 1e-3 \
