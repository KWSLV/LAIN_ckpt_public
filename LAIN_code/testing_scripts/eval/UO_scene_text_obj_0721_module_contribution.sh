...
# Post-hoc module contribution evaluation for 0721 checkpoints.
#
# Restores the original 0721 architecture:
#   last / V32 / Text r8-a8 / Object r4 /
#   SceneGate v5 tanh, alpha 0.1, H128, init_std 1e-3.
# It evaluates full, SceneGate OFF, and Text OFF sequentially.
# Object Adapter remains enabled and is not ablated.

if [[ $# -ne 1 ]]; then
  echo "Usage: bash $0 /absolute/path/to/0721_checkpoint.pt" >&2
  exit 2
fi
...
OUTPUT_DIR="${OUTPUT_DIR:-/root/autodl-tmp/Lain/UO_scene_text_obj_run/0721_module_contribution_eval}"
...
torchrun \
  ...
  --scene_gate_alpha 0.1 \
  --scene_gate_activation tanh \
