...
class LAINadapter(nn.Module):
    ...
    def forward(self, x, prior=None, la_masks=None):
        ...
        scene_box = object_boxes.new_tensor([[0, 0, 224, 244]])
        object_boxes = torch.cat([object_boxes, scene_box], dim=0)  # human
        ...
        sentinel = human_pid.new_full((1,), -1)
        object_pid = torch.cat([object_pid, sentinel])
        human_pid = torch.cat([human_pid, sentinel])
        human_object_pid = torch.cat([human_pid, object_pid])
        object_human_pid = torch.cat([object_pid, human_pid])
        ...
        for z, layer in enumerate(self.mhsa_layers3):
            ip_feats = layer(context_tokens[:, human_object_pid],
                             context_tokens[:, object_human_pid],
                             tgt_mask=None,
                             tgt_key_padding_mask=None,
                             memory_key_padding_mask=None,
                             pos=None, query_pos=None)
