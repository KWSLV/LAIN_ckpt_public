...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash scripts/training/UO_scene_gate_refine.sh /absolute/path/to/best_checkpoint.pt"
  exit 2
fi
...
# Preserve the best adapter/prompt weights, reset SceneGate, and train only it.
# Per-image standardization prevents both common-bias and scale saturation.
...
torchrun --rdzv_id "$RDZV_ID" --rdzv_backend c10d --nproc_per_node 1 \
  ...
  --init_from "$BASE_CHECKPOINT" --reset_scene_gate_on_load \
  ...
  --seed "${SEED:-66}" --weight-decay 1e-4 --lr-drop 10 \
  --lr-head "${GATE_LR:-1e-3}" \
