...
def main(rank, args):
    ...
    if getattr(args, 'unseen_lr_threshold_schedule', False):
        ...
        if not getattr(args, 'disable_lr_scheduler', False):
            raise ValueError(
                '--unseen-lr-threshold-schedule requires --disable-lr-scheduler '
                'so StepLR cannot apply an additional unrequested reduction.'
            )
        if args.unseen_lr_threshold <= 0:
            raise ValueError('--unseen-lr-threshold must be positive.')
        if not 0 < args.unseen_lr_threshold_factor < 1:
            raise ValueError('--unseen-lr-threshold-factor must be in (0, 1).')
...
if __name__ == '__main__':
    ...
    if local_rank == 0:
        ...
        wandb_resume_from = os.environ.get(
            'WANDB_RESUME_FROM', ''
        ).strip()
        if wandb_resume and wandb_resume_from:
            raise ValueError(
                'WANDB_RESUME and WANDB_RESUME_FROM are mutually exclusive.'
            )
        if wandb_run_id:
            wandb_init_kwargs['id'] = wandb_run_id
        if wandb_resume_from:
            wandb_init_kwargs['resume_from'] = wandb_resume_from
        elif wandb_resume:
            wandb_init_kwargs['resume'] = wandb_resume
        if (
            getattr(args, 'lain_object_staged_retry_policy', False)
            and getattr(args, 'staged_resume_existing_run', False)
        ):
            # On resumed long epochs, frequent system-stat file-stream chunks
            # can suppress W&B's explicit heartbeat while failing to refresh
            # the server-side heartbeat timestamp. Keep system metrics, but
            # sample them less frequently than the heartbeat interval so the
            # backend receives an explicit keepalive and does not mark an
            # active training process as crashed.
            wandb_init_kwargs['settings'] = wandb.Settings(
                heartbeat_seconds=30,
                x_stats_sampling_interval=60.0,
            )
            print(
                '[W&B Resume] heartbeat=30s, system-stats interval=60s'
            )
