...
if [[ -n "$RESUME_CHECKPOINT" ]]; then
  if [[ ! -f "$RESUME_CHECKPOINT" ]]; then
    echo "[ERROR] Missing staged resume checkpoint: $RESUME_CHECKPOINT" >&2
    exit 1
  fi
  LOAD_ARGS=(--resume "$RESUME_CHECKPOINT" --staged-resume-existing-run)
else
...
if [[ -n "$RESUME_CHECKPOINT" ]]; then
  echo "[$SCRIPT_NAME] Resume staged run from: $RESUME_CHECKPOINT"
else
