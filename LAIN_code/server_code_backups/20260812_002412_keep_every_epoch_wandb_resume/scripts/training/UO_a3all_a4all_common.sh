...
if (( available_kb < minimum_kb )); then
  ...
  echo "[ERROR] A 20-epoch milestone run is not started." >&2
  exit 1
fi
...
expected = {
    ...
    "keep_epoch_checkpoints": [5, 10, 15, 20],
...
verify_outputs() {
  ...
  for epoch in 5 10 15 20; do
    if [[ -z "$(find_epoch_checkpoint "$epoch" || true)" ]]; then
      echo "[ERROR] ${VARIANT} is missing the epoch-$epoch milestone checkpoint." >&2
      return 1
    fi
  done
...
echo "[${VARIANT^^}] Checkpoints: epochs 5/10/15/20 + best_unseen.pt"
...
torchrun \
  ...
  --keep-epoch-checkpoints 5 10 15 20 \
