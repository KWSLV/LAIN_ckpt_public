...
SESSION_NAME="${SESSION_NAME:-lain_uo_unseen_$TIMESTAMP}"
...
# Environment variables such as SEED, EPOCHS and BATCH_SIZE are inherited.
TRAIN_COMMAND="cd '$ROOT' && RUN_ROOT='$RUN_ROOT' OUTPUT_DIR='$CHECKPOINT_DIR' bash scripts/training/UO_unseen_tuned.sh 2>&1 | tee -a '$LOG_FILE'"
