...
class CustomisedDLE(DistributedLearningEngine):
    ...
    def save_checkpoint(self):
        """Save a checkpoint and optionally rotate older checkpoints."""
        super().save_checkpoint()

        keep = int(getattr(self.args, 'keep_last_checkpoints', 0) or 0)
        if self._rank != 0 or keep <= 0:
            return
        ...
        checkpoints.sort()
        for _, _, filename in checkpoints[:-keep]:
