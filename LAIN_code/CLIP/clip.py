...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor,mask=None, prior=None, la_masks=None):
        ...
        '''
        if self.adapter and prior is not None and la_masks is not None:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prior=None, query=None, mask=None, la_masks=None, return_cls: bool = False):
        bs, c, h, w = x.shape
        ...
        ) + self.positional_embedding.to(x.dtype)[:1, :].unsqueeze(0)
        image_tokens = x + self.positional_embedding.to(x.dtype)[1:, :].unsqueeze(0)

        if query is None:
            x = torch.cat([cls_token, image_tokens], dim=1)
        else:
            if query.dim() == 2:
                query = query.unsqueeze(0).expand(bs, -1, -1)
            x = torch.cat([query, cls_token, image_tokens], dim=1)
        ...
        patch_tokens = (h // self.patch_size) * (w // self.patch_size)
        prefix_tokens = x[:, :-patch_tokens, :]
        local_tokens = x[:, -patch_tokens:, :]
        local_feat = local_tokens.view(bs, h // self.patch_size, w // self.patch_size, -1).permute(0, 3, 1, 2)

        if return_cls:
            if query is None:
                return prefix_tokens[:, :0, :], prefix_tokens[:, 0, :], local_feat
            return prefix_tokens[:, :-1, :], prefix_tokens[:, -1, :], local_feat

        return prefix_tokens, local_feat
