...
SESSION_NAME="${SESSION_NAME:-lain_uo_$TIMESTAMP}"
...
TRAIN_COMMAND="cd '$ROOT' && RUN_ROOT='$RUN_ROOT' OUTPUT_DIR='$CHECKPOINT_DIR' bash scripts/training/UO.sh 2>&1 | tee -a '$LOG_FILE'"
tmux new-session -d -s "$SESSION_NAME" "$TRAIN_COMMAND"

sleep 1
if ! tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
  echo "[ERROR] Training session exited immediately. Check: $LOG_FILE" >&2
  exit 1
fi
...
echo "[INFO] Detach from tmux: press Ctrl-b, then d"
